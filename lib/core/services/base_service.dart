  ...
  Future<ApiResponse<T>> executeRequest<T>(
    ...
    try {
      ...
      return ApiResponse(
        success: false,
        data: null as T,
        error: ErrorData.networkError(e.toString()),
      );
    }
  }
  ...
  ApiResponse<T> handleError<T>(
    ...
    if (error.response != null) {
      ...
      if (responseData is Map<String, dynamic>) {
        errorMessage = responseData['message'] ?? responseData['error']?['message'] ?? 'Server error ($statusCode)';
      } else {
    ...
    return ApiResponse(
      success: false,
      data: null as T,
      error: ErrorData.networkError(errorMessage),
    );
  }
