...
class ApiResponse<T> {
  ...
  const ApiResponse({required this.success, required this.data, required this.error});
...
class ErrorData {
  ...
  factory ErrorData.networkError(String details) => ErrorData(code: 'NETWORK_ERROR', message: 'Network error occurred', details: details);

  /// Create an unauthorized error
  factory ErrorData.unauthorized() => ErrorData(code: 'UNAUTHORIZED', message: 'Authentication required', details: 'No token found');

  /// Create a generic error
  factory ErrorData.generic(String message, {String? details}) => ErrorData(code: 'GENERIC_ERROR', message: message, details: details ?? '');
}
